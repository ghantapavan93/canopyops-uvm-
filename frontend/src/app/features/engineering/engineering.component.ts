...
export class EngineeringComponent implements OnDestroy {
  ...
  readonly tests: Section = {
    ...
    items: [
      ...
      { label: 'Cypress e2e — 10 passing (8 specs)', ok: true, detail: 'critical journey (plan → offline execution → partial upload → conflict recovery → evidence retry → verification → targeted follow-up → close → Proof Pack); the command palette (Ctrl/Cmd-K open, type-to-filter, keyboard + header-button navigation, Escape); the risk sign-off lifecycle (RBAC gate → certified reviewer signs off → append-only history → revoke reopens the span); the compliance report (rollup render, circuit + activity-date scoping, real PDF export); the OData $batch panel (several reads in a single POST, per-id statuses); the reliability-outcome panel (closed-vs-effective with per-circuit SAIDI movement); vegetation intelligence (the hot-spotting heat list → driver breakdown, and the cycle-buster watchlist filter); and the not-found route (real 404 with a path back into the console). Runs headless (Electron) against the running stack via `npm run e2e`; verified green (~10s). The service worker is skipped under Cypress so it can never intercept e2e navigations.' },
    ],
  };
