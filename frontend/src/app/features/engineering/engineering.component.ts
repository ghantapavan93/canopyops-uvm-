...
export class EngineeringComponent implements OnDestroy {
  ...
  readonly tests: Section = {
    ...
    glyph: '✓',
    items: [
      { label: 'Backend API (pytest) — 77 passing', ok: true, detail: 'idempotent replay, revision conflict (409) + resolve-under-same-key, evidence-completeness gate, RBAC (403), PostGIS coverage math, full plan→verify→close loop; plus plan creation + validation (422 structured envelope), overview periods, stewardship real signals, choropleth, geometry analysis, OpenAPI, GeoJSON import, metrics endpoint, pagination, the OData integration surface ($metadata, $filter/$select/$expand, deferred nav, ETag/304, and $batch — many reads in one round-trip with dependsOn 424 short-circuiting and a read-only 501 guard), the activity-date-scoped compliance rollup, reliability (statement_timeout cancels a runaway query, the in-flight limiter sheds load with 503 while probes stay answerable, a per-client token bucket returns 429 over its burst, the DB circuit breaker opens→half-opens→closes and fast-fails DB routes while it\'s open, /metrics exposes the whole surface), geofence proximity alerts (PostGIS ST_Distance/ST_Contains → clear/warning/breach), the versioned offline zones snapshot (ETag/304), and the 3D terrain DEM grid + corridor elevation/slope profile' },
