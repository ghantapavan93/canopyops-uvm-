...
export class EngineeringComponent implements OnDestroy {
  ...
  readonly tests: Section = {
    ...
    items: [
      ...
      { label: 'Cypress e2e — 4 passing (2 specs)', ok: true, detail: 'critical journey (plan → offline execution → partial upload → conflict recovery → evidence retry → verification → targeted follow-up → close → Proof Pack); plus the command palette (Ctrl/Cmd-K open, type-to-filter, keyboard + header-button navigation, Escape). Runs headless (Electron) against the running stack via `npm run e2e`; verified green (~7s). The service worker is skipped under Cypress so it can never intercept e2e navigations.' },
    ],
  };
