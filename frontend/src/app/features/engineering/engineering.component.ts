...
export class EngineeringComponent implements OnDestroy {
  ...
  readonly tests: Section = {
    ...
    glyph: '✓',
    items: [
      { label: 'Backend API (pytest) — 84 passing', ok: true, detail: 'idempotent replay, revision conflict (409) + resolve-under-same-key, evidence-completeness gate, RBAC (403), PostGIS coverage math, full plan→verify→close loop; plus plan creation + validation (422 structured envelope), overview periods, stewardship real signals, choropleth, geometry analysis, OpenAPI, GeoJSON import, metrics endpoint, pagination, the OData integration surface ($metadata, $filter/$select/$expand, deferred nav, ETag/304, and $batch — many reads in one round-trip with dependsOn 424 short-circuiting and a read-only 501 guard), the activity-date-scoped compliance rollup, the reliability-outcome model (per-circuit closed-vs-effective with synthetic SAIDI/SAIFI/CAIDI/CMI driven by real coverage/evidence/status — CAIDI=SAIDI/SAIFI, CMI=SAIDI×customers, deterministic, rollup totals reconcile), reliability (statement_timeout cancels a runaway query, the in-flight limiter sheds load with 503 while probes stay answerable, a per-client token bucket returns 429 over its burst, the DB circuit breaker opens→half-opens→closes and fast-fails DB routes while it\'s open, /metrics exposes the whole surface), geofence proximity alerts (PostGIS ST_Distance/ST_Contains → clear/warning/breach), the versioned offline zones snapshot (ETag/304), and the 3D terrain DEM grid + corridor elevation/slope profile' },
      ...
      { label: 'Cypress e2e — 9 passing (7 specs)', ok: true, detail: 'critical journey (plan → offline execution → partial upload → conflict recovery → evidence retry → verification → targeted follow-up → close → Proof Pack); the command palette (Ctrl/Cmd-K open, type-to-filter, keyboard + header-button navigation, Escape); the risk sign-off lifecycle (RBAC gate → certified reviewer signs off → append-only history → revoke reopens the span); the compliance report (rollup render, circuit + activity-date scoping, real PDF export); the OData $batch panel (several reads in a single POST, per-id statuses); the reliability-outcome panel (closed-vs-effective with per-circuit SAIDI movement); and the not-found route (real 404 with a path back into the console). Runs headless (Electron) against the running stack via `npm run e2e`; verified green (~10s). The service worker is skipped under Cypress so it can never intercept e2e navigations.' },
    ],
  };
