...
export class EngineeringComponent implements OnDestroy {
  ...
  readonly tests: Section = {
    ...
    items: [
      ...
      { label: 'Cypress e2e — 6 passing (4 specs)', ok: true, detail: 'critical journey (plan → offline execution → partial upload → conflict recovery → evidence retry → verification → targeted follow-up → close → Proof Pack); the command palette (Ctrl/Cmd-K open, type-to-filter, keyboard + header-button navigation, Escape); the risk sign-off lifecycle (RBAC gate → certified reviewer signs off → append-only history → revoke reopens the span); and the compliance report (rollup render, circuit scoping, real PDF export). Runs headless (Electron) against the running stack via `npm run e2e`; verified green (~10s). The service worker is skipped under Cypress so it can never intercept e2e navigations.' },
    ],
  };
