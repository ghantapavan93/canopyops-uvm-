...
export class EngineeringComponent implements OnDestroy {
  ...
  readonly boundaries: Section = {
    ...
    items: [
      ...
      { label: 'Tenant (program) isolation', detail: 'Every program-owned row is auto-scoped by a SQLAlchemy Session filter driven by the JWT — a forgotten WHERE can\'t leak data. One program can never read or fetch another\'s records (404, not 403). DB-level RLS is documented as the production defense-in-depth (docs/MULTI-TENANCY.md).' },
