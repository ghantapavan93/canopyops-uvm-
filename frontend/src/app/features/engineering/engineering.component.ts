...
export class EngineeringComponent implements OnDestroy {
  ...
  /** Counts come from the generated evidence — never typed. These section
   *  labels were missed when the tiles were fixed and went on claiming 136
   *  pytest / 17 Cypress against a real 153 / 22, on the one screen whose
   *  whole job is being true. scripts/check_type_floor.py now bans the
   *  hand-typed form outright. */
  private count(key: string): number {
    return testEvidence.suites.find((s) => s.key === key)?.passed ?? 0;
  }
  ...
  readonly architecture: Section = {
    ...
    items: [
      ...
      { label: 'Data', detail: 'PostgreSQL 16 + PostGIS: server-side spatial filtering (ST_Intersects, ST_MakeEnvelope), GIST indexes on all geometry columns. Evidence files live in S3-compatible object storage (MinIO) — not the DB — behind presigned upload/download URLs. Row-Level Security is enforced at the database: the app connects as a non-superuser role and per-program policies key on a transaction GUC.' },
  ...
  readonly boundaries: Section = {
    ...
    items: [
      ...
      { label: 'Tenant (program) isolation — two layers', detail: 'App layer: every program-owned row is auto-scoped by a SQLAlchemy Session filter driven by the JWT — a forgotten WHERE can\'t leak. DB layer: Postgres Row-Level Security is ENABLED — the API/worker connect as a non-superuser role and policies key on a per-transaction GUC, so even a raw query can\'t cross programs (a superuser bypasses RLS; migrations/seed use it). One program can never read another\'s records (404, not 403). See docs/MULTI-TENANCY.md.' },
